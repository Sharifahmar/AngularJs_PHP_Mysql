...
//        
    ...
    private function getSubCategory() {
        ...
        if(isset($_GET['category_name'])){
            ...
            $this->db->initateRawQuery('SELECT  B.id as sub_cat_id ,A.form_type as category_name , B.sb_category_name as sub_category_name FROM form_type AS A , sub_categories AS B
            WHERE A.fid=B.ft_id AND A.form_type='.$cname.'');
...
//        
        ...
        $this->error = 'Not able to fetch sub category list';
        return $this->response($this->getJson(), 500);
    }
...
/**
 * Old Method Comment out not in Use
 */
...
*/
    private function searchForms() {
        ...
        if(isset($_GET['category_name']) && isset($_GET['sub_category_name'])){
            $cname = $_GET['category_name'];
            $sbcname = $_GET['sub_category_name'];

            $this->db->initateRawQuery("SELECT * FROM main_form WHERE category_name LIKE '%{$cname}%'  AND sub_category_name LIKE '%{$sbcname}%'");
        ...
        if ($res['rows']) {
            ...
            foreach ($languages_raw as $key => $value) {
                ...
                $form_type_decoded = json_decode($value['form_type'], true);
                $language_id_decoded = json_decode($value['language_id'], true);
                $admin_decoded = json_decode($value['admin'], true);

                $languages_formatted[$key] = [
                    'id' => $value['id'],
                    'form_name' => $value['form_name'],
                    'form_uid' => $value['form_uid'],
                     'category' => $form_type_decoded,
                    'sub_category' => $state_decoded,
                    'dept_name' => $dept_decoded,                 
                    'admin' => $admin_decoded,
                    'language_id' => $language_id_decoded,
                    'fileName' => $value['fileName'],
...
//        
    ...
    private function innerAttributeing() {
